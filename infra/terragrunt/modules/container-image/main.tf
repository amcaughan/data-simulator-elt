locals {
  hashed_dir_files = flatten([
    for dir_path in var.hash_dirs : [
      for relative_path in fileset(dir_path, "**") : "${dir_path}/${relative_path}"
      if !endswith(relative_path, "/")
    ]
  ])
  all_hash_inputs = sort(distinct(concat(local.hashed_dir_files, var.hash_files)))
...
resource "terraform_data" "build_and_push" {
  ...
  provisioner "local-exec" {
    command = "${path.module}/build_and_push_image.sh ${var.aws_region} ${var.repository_url} ${local.image_tag} ${var.dockerfile_path} ${var.build_context_dir}"
  }
}
